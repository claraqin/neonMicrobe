...
# takes 5.5 minutes to run if not previously downloaded
dat_soil <- downloadRawSoilData(sites = 'all', startYrMo = "2013-06", endYrMo = "2020-07", return_data=TRUE, overwrite=FALSE)
saveRDS(soilDat, "soilDat.rds")

# takes 4 minutes (even if previous downloaded)
dat_sequenceMetadata <- downloadSequenceMetadataRev(sites = 'all', startYrMo = "2013-06", endYrMo = "2020-07", targetGene = "ITS", dir = PRESET_OUTDIR_SOIL)
...
dat_soil <- rbind(dat_soil, c_n_merged) # merge back together
...
saveRDS(out, file = "soilDatabase.rds")
