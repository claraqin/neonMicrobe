...
for(i in 1:length(zipF[2:6])) { # <-------------------------- TODO: CHANGE TO BE FOR ALL FILES
  ...
  unzippedF <- grep("/(?!run)[^/]*fastq$", list.files(path = BASE_DIR, recursive=TRUE, full.names=TRUE), perl=TRUE, value=TRUE)
  
  # rename all unzipped files by appending sequencer run ID
  unzippedF_rename <- file.path(BASE_DIR, paste0("run", runID, "_", basename(unzippedF)))
