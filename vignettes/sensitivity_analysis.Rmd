...
if(length(rawFs) > N_SAMPLES) {
  ...
  for(i in 1:length(runIDs)) {
    ...
    if(length(rawFs_runID) > 5) {
      rawFs_subset <- c(rawFs_subset, rawFs_runID[1:round(N_SAMPLES/length(runIDs))])
      rawRs_subset <- c(rawFs_subset, rawRs_runID[1:round(N_SAMPLES/length(runIDs))])
...
Ordinate samples from different sequencing runs together:

```{r}
for(i in 1:length(runIDs)) {
  message(runIDs[i])
  print(unique(get_variable(ps_runID[[i]], "siteID")))
  # print(unique(get_variable(ps_runID[[i]], "dataQF.seq")))
}
```

```{r}
# HARV_runs <- c("runB69RN", "runC8VMV", "runC983L")
# HARV_runs <- c("runC8VMV", "runC983L")
# select_runs <- c("runC24VW", "runCDHG2", "runCDJ5J") # contain sites TALL, STER, OAES
select_sites <- c("TALL", "STER", "OAES")
select_runs <- c("runCDHG2", "runCDJ5J")
```

```{r}
ps_select <- subset_samples(physeq_joined_nonzero, runID %in% select_runs & siteID %in% select_sites)
set.seed(1010101)
ord_select <- ordinate(ps_select, "NMDS", "bray", k=2)
saveRDS(ord_select, file.path(PATH_TEST, "results", paste0("sensitivity_ordination_select_runs.Rds")))
```


```{r}
sampledata <- as(sample_data(ps_select), "data.frame")
sampledata$maxEE.R <- as.factor(sampledata$maxEE.R)
sampledata$truncLen.R <- as.factor(sampledata$truncLen.R)
sampledata <- cbind(sampledata, scores(ord_select)[match(rownames(sampledata), rownames(scores(ord_select))),])
ggplot(sampledata, aes(x=NMDS1, y=NMDS2)) +
  # geom_point(aes(col=truncLen.R, shape=maxEE.R)) +
  # geom_point(aes(col=plotID, shape=maxEE.R)) +
  geom_point(aes(col=truncLen.R, shape=runID)) +
  stat_ellipse(aes(group=siteID), lwd=0.1, col="grey20") +
  scale_shape_manual(values=c(21, 22, 24)) +
  # guides(col=FALSE, shape=FALSE) +
  ggtitle(runIDs[i]) +
  # geom_text(data=stress_annotations[[i]], aes(x=xloc, y=yloc, label=label, hjust=hjust, vjust=vjust), size=3) +
...
#   # geom_point(aes(col=truncLen.R, shape=maxEE.R)) +
#   # geom_point(aes(col=plotID, shape=maxEE.R)) +
#   geom_point(aes(col=sequencerRunID, shape=maxEE.R)) +
#   stat_ellipse(aes(group=runID), lwd=0.1, col="grey20") +
#   scale_shape_manual(values=c(21, 22, 24)) +
#   # guides(col=FALSE, shape=FALSE) +
#   # geom_text(data=stress_annotations[[i]], aes(x=xloc, y=yloc, label=label, hjust=hjust, vjust=vjust), size=3) +
#   NULL
