...
# N_SAMPLES <- 210
N_SAMPLES <- 30
DIRNAME_TEST <- "qf_test_1-12-2021"
# runIDs <-  c("runB69PP", "runB69RN", "runB9994", "runBDNB6", "runBF462", "runBFDG8", "runBNMJ5", "runBNMWB", "runBRPH4", "runBTP49", "runC24VW", "runC25T6", "runC5B2R", "runC7WK3", "runC8VMV", "runC977L", "runC983L", "runCBJYB", "runCBTWG", "runCDHG2", "runCDJ5J")
...
```{r, eval=FALSE}
seqtabs_joinrun <- readRDS(file.path(PATH_16S, "qf_test_12-30-2020", "results", "sensitivity_seqtabs_joinrun_list.Rds"))
track <- read.csv(file.path(PATH_16S, "qf_test_12-30-2020", "results", "sensitivity_trackReads.csv"), row.names=1)
```
...
for (i in 1:length(runIDs)) {
  physeq_joined_nonzero_star <- subset_samples(physeq_joined_nonzero, runID==runIDs[i])
  set.seed(1010101)
  ord_list[[i]] <- ordinate(physeq_joined_nonzero_star, "NMDS", "bray", k=2)
...
for(i in 1:length(ord_list)) {
  sample_data(subset_samples(physeq_joined_nonzero, runID==runIDs[i]))
}
...
plot_ordination(physeq_joined_nonzero_runC5B2R, ordination, type="samples",
  ...
  NULL
ggsave(file.path(PATH_TEST, "results", "sensitivity_ordination_runC5B2R.png"), width=5, height=3.5, units="in")
```

What if we removed samples with `truncLen.R` = 160?

```{r, fig.width=5, fig.height=3.5}
physeq_joined_nonzero_runC5B2R_hitrunc <- subset_samples(physeq_joined_nonzero_runC5B2R, truncLen.R!=160)
set.seed(1010101)
ordination_runC52BR_hitrunc <- ordinate(physeq_joined_nonzero_runC5B2R_hitrunc, "NMDS", "bray", k=2)

plot_ordination(physeq_joined_nonzero_runC5B2R_hitrunc, ordination_runC52BR_hitrunc, type="samples",
                col="truncLen.R.factor", shape="maxEE.R.factor") +
  stat_ellipse(aes(group=sampleID), lwd=0.1, col="grey20") +
  scale_shape_manual(values=c(21, 22, 24)) +
  # coord_cartesian(xlim=c(-0.096, -0.09), ylim=c(-0.002, 0.001)) +
  NULL
ggsave(file.path(PATH_TEST, "results", "sensitivity_ordination_runC5B2R_hitrunc.png"), width=5, height=3.5, units="in")
```
