---
title: "Convert ITS data to Phyloseq with metadata"
...
```{r}
# Load sequence table and taxonomic table
seqtab_orig <- readRDS("./data/NEON_ITS_seqtab_nochim.rds")
taxa_orig <- readRDS("./data/NEON_ITS_tax.rds")
```
...
sampledata <- sampledata_full %>% filter(targetGene == "ITS") %>% 
...
sampledata <- merge(sampledata, soils, all.x)
...
```{r}
saveRDS(ps, "./data/NEON_ITS_phyloseq.rds")
```