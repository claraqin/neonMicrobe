...
for (i in 1:length(unique_runs)) {
  ...
  fn <- meta_fn$file[meta_fn$sequencerRunID==runID]
  fn_base <- basename(fn)

  # Trim reads based on the primer lengths supplied in params.r
  # trim_trackReads <- trimPrimers16S(fn, PATH_TRIMMED, "CCTACGGGNBGCASCAG", "GACTACNVGGGTATCTAATCC")
  ...
  # filter_trackReads <- qualityFilter16S(fn_base, PATH_TRIMMED, PATH_FILTERED, MULTITHREAD, maxEE = c(MAX_EE_FWD, MAX_EE_REV), truncLen=220)
  ...
  seqtab.list <- runDada16S2(file.path(PATH_FILTERED, fn_base), meta, multithread = MULTITHREAD, verbose = VERBOSE)
